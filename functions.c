...
struct pop_entry * read_data(char * PATH){
	...
	struct stat info;
	stat(PATH, &info);
	struct pop_entry * e_arr = malloc(info.st_size / sizeof(struct pop_entry));
...
void add_data (char * PATH){
	char DATA[256];
	printf("Enter in data entry in the format of year borough population\n");
	
	struct pop_entry e;
	fgets(DATA, 256, stdin);
	sscanf(DATA, "%d %s %d", &e.year, e.boro, &e.pop);
